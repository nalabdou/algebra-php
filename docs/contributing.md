...
├── Contract/                   ← Interfaces — the stable API surface
...
│   ├── Join/                   ← Join types
│   ├── Set/                    ← Set operations
│   ├── Aggregate/              ← (groupBy, aggregate, tally, partition)
│   ├── Window/                 ← (window dispatcher, movingAvg, normalize)
│   └── Utility/                ← Structural operations
├── Aggregate/                  ← Aggregate functions in 4 namespaces
├── Planner/                    ← QueryPlanner + 4 optimization passes
├── Adapter/                    ← Adapters (array, generator, traversable)
